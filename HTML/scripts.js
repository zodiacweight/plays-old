...
var p_aboutcharacters, Extradecomposers, maniac, headers;
...
xhr.onload = function () {
    ...
    if (xhr.status != 200) {
        ...
        p_aboutcharacters = data.p_aboutcharacters;
        Extradecomposers = data.ExtraDecomposers;
        maniac = data.Maniac;
        headers = data.headers;
        // вывести результат
        console.log(p_aboutcharacters); // responseText -- текст ответа.
    }
};
            ...
            for (var index in replics[this.id]) { // пробег по partn (replics[this.id]) от arrayElementObject
                ...
                if (subjectName == "image") {
                    ...
                    if (!(subjectName in presrolesobject) && subjectName.indexOf('&') < 0 && subjectName.indexOf('letter') < 0) {
                       // console.log('case: authorwords', 'subjectName = '+subjectName);
                        console.log('3 условия выполняются');
                        ...
                        if (name_in_checkbox == "Author's words") {
                            ...
                            for (rundivs = 0; rundivs < length_charwords; rundivs++) {
                                ...
                                if (name_in_h4.indexOf(" &") == -1) {
                                    ...
                                    /*Если name_in_h4 содержит в себе амперсанты, то преобразовывается в массив, и вместо пробел, амперсант,
                                     * пробел - разделитель между элементами массива. Внутренний цикл пробегает по этому массиву, проверяет
                                     * каждый элемент на присутствие в объекте checkedroles. Если ни один из элементов массива не присутствует, то
                                     * реплика теряет класс. Если попадается присутствующий, то реплика не теряет класс, и цикл прерывается.
                                     * */
...
function loadAboutCharacters(keyAboutCharacters) {
    if (roleslist.innerHTML != "") {
        roleslist.innerHTML = "";
    }
    text.innerHTML = "<h2>About characters</h2>" + p_aboutcharacters[keyAboutCharacters];
    area_forcontent.style.borderLeft = "3px solid #345693";
    contentlist.style.borderRight = "none";
}
