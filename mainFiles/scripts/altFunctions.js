...
function handleData(path, callback){
...
function setComponentsOfBeginning (selectedPlay) {
    console.log('selectedPlay:', selectedPlay);
    mainImage.innerHTML = "<h2>"+selectedPlay["onTheBiginning"]["header"]+"</h2>"+selectedPlay["onTheBiginning"]["images"][0];
    littleImages.innerHTML=selectedPlay["onTheBiginning"]["images"][0];
    document.getElementById("preview").getElementsByTagName("P")[0].innerText=selectedPlay["onTheBiginning"]["text"];
    for (var addLittleImages=1; addLittleImages < selectedPlay["onTheBiginning"]["images"].length; addLittleImages++) {
        littleImages.innerHTML+=selectedPlay["onTheBiginning"]["images"][addLittleImages];
    }
    changeMainImage();
}
...
function setClickToLoadPart(part, choicedPlays) {
    parts_with_numbers = Object.keys(choicedPlays["Parts"]);
    var curPart;
    for (part in choicedPlays["Parts"]) {
        ...
        curPart.onclick = function () // назначается обработчик события -- ВНУТРИ ЦИКЛА!
        {
            if (toChooseRoles.innerHTML=="") {
                ...
                var top_of_play = document.getElementById("top_of_play");
                top_of_play.innerHTML = "<div><h2 id='headerForPlay'>" + this.id + " " + choicedPlays.headers[this.id] +
                    "</h2></div>" + "<div id='buttons'><input type='button' value='<' id='scrollBack'><input type='button' value='>' id='scrollFront'>" +
                ...
                PartNumber = this.innerText;
                alert(PartNumber===undefined);
...
function changePart (idOfPlay, choicedPlays, content_of_play) {
    ...
    for (var index in choicedPlays[idOfPlay]) { // choicedPlays[idOfPlay] - part, choicedPlays[idOfPlay][index] - объект-реплика
                ...
                switch (subjectName) {
                    ...
                    default:
                        if ((idOfPlay == "Part 1.2")&&(subjectName=="Beatrix")) {
                            listOfCheckboxes.innerHTML +="<p><input type='checkbox' class='checkcharacter'>" + subjectName+
